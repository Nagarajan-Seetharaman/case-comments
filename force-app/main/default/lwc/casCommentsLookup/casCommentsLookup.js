...
export default class CustomLookup extends LightningElement {
    ...
    @api needonload=false;
    @api emAddresses =[];
 
    connectedCallback() {
        console.log('onloadneed: '+this.needonload);
        if (this.needonload) {
            this.prepareEmailForEditFunc();
        }        
    }

    prepareEmailForEditFunc(){
        if(this.emAddresses!=undefined && this.emAddresses.length>0){
            this.emAddresses.forEach(addr =>  {
                let prepareAddress ={};
                prepareAddress={
                    'Id': addr,
                    'Name': addr,
                ...
                this.selectedRecord.push(prepareAddress);
            });
            this.showPill = true;
        }
        console.log('prepareEmailForEditFunc : '+this.selectedRecord);
    }
    ...
    emailAddressSelectionHandler(value){ 
        ...
        }else if(this.uniqueName=='ToAddressEdit'){
            const toAddrEdit = new CustomEvent('emailtoselectionedit',{
                'detail': {selectedRecord: emailAddr}
            });
            this.dispatchEvent(toAddrEdit);
        }else if(this.uniqueName=='CcAddressEdit'){
            const ccAddrEdit = new CustomEvent('emailccselectionedit',{
                'detail': {selectedRecord: emailAddr}
            });
            this.dispatchEvent(ccAddrEdit);
        }else if(this.uniqueName=='BccAddressEdit'){
            const bccAddrEdit = new CustomEvent('emailbccselectionedit',{
                'detail': {selectedRecord: emailAddr}
            });
            this.dispatchEvent(bccAddrEdit);
        }
    }
